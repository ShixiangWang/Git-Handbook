...
版本库(Repository)是版本控制系统用来存放所有历史数据的地方，主要**存放各个文件的当前状态，历史修改时间，谁做的修改，以及修改的原因**。举个简单的例子，就好比银行的保险箱，每次往里存钱，都会记录谁，什么时间，存放多少钱，存入的原因等。对应的版本库（Repository）主要存放代码（文档，数据，图标等），并且每一次更新都要记录谁，什么时间，提交了什么更新，以及更新的原因是什么。
...
Linus花了两周时间自己用C写了一个分布式版本控制系统，这就是git！一个月之内，Linux系统的源码便由git管理了！牛是怎么定义的呢？大家可以体会一下。
...
其他系统安装参考[这个文档](http://www.liaoxuefeng.com/wiki/0013739516305929606dd18361248578c67b8067c8c017b000/00137396287703354d8c6c01c904c7d9ff056ae23da865a000)。
...
`pwd` 命令用于显示当前目录。例如，在我的电脑，这个仓库位于 `/Users/michael/learngit` 。
...
当我们知道对文件作了什么修改后，再把它提交到仓库就放心多了，添加修改文件和提交修改是两步，第一步是 `git add` ，第二步是 `git commit`。在提交修改之前，我们可以再运行git status看看当前仓库的状态。
